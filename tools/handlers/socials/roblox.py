...
class RobloxUser(commands.Converter):
  async def convert(self, ctx: commands.Context, argument: str) -> Roblox:
    async with aiohttp.ClientSession(headers={"Authorization": ctx.bot.pretend_api}) as session: 
     async with session.get("https://api.pretend.best/roblox", params={"username": argument}) as r: 
       match r.status: 
         case 404: 

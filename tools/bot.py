...
class Pretend(commands.AutoShardedBot):
  ...
  def __init__(self, db: asyncpg.Pool=None):
      ...
      async def after_invoke(ctx):
        posthog.capture(ctx.author.id, event='command executed', properties={"command name": ctx.command.qualified_name}, groups={"guild": ctx.guild.id})
  ...
  def public_cogs(self) -> List[commands.Cog]: 
   ...
   return [c for c in self.cogs if not c in ['Jishaku', 'Owner', 'Members', 'Auth', 'Reactions', 'Messages']]
  ...
  async def get_context(self, message: discord.Message, cls=PretendContext) -> PretendContext:
   """
  ...
  async def get_prefixes(self, message: discord.Message) -> Set[str]:
   ...
   prefixes = set()
   r = await self.db.fetchrow('SELECT prefix FROM selfprefix WHERE user_id = $1', message.author.id) 
   if r: 
    prefixes.add(r[0])
   re = await self.db.fetchrow("SELECT prefix FROM prefixes WHERE guild_id = $1", message.guild.id)
   if re: 
    prefixes.add(re[0])
   else:
    prefixes.add(";") 
  ...
  async def on_message(self, message: discord.Message) -> Any: 
   if not message.author.bot and message.guild:
      ...
      if perms.send_messages and perms.embed_links: 
        if not await self.db.fetchrow("SELECT * FROM blacklist WHERE id = $1 AND type = $2", message.author.id, "user"):   

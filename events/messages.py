...
class Messages(Cog):  
    ...
    if not cooldown: 
      async with self.locks[message.guild.id]:
        async with message.channel.typing():
            ...
            async with aiohttp.ClientSession(headers=headers) as cs:
              async with cs.post("https://fastdl.app/c/", data=body) as r:
                if r.status == 200:
                  ...
                  post_data = {
                    "url": post['href'],
                    "extension": "png" if post['data-mediatype'] == "Image" else "mp4"
                  }

                  await self.bot.cache.set(f"igpost-{url}", post_data, 3600)
                else: 
          ...
          file = File(await self.bot.getbyte(post_data['url']), filename=f"pretend_instagram.{post_data['extension']}")
   ...
   async def repost_tiktok(self, message: Message):
      ...
      if not cooldown: 
        async with self.locks[message.guild.id]:  
         ...
         async with message.channel.typing():       
          x = await self.bot.session.get_json("https://tikwm.com/api/", params={"url": url}) 
          if x['data'].get("images"):
            ...
            video = x["data"]["play"]
            file = File(fp=await self.bot.getbyte(video), filename="pretendtiktok.mp4")
            embed = Embed(
              color=self.bot.color,
      ...
      if message.interaction.name == "bump" and message.author.id == 302050872383242240:   
        if "Bump done!" in message.embeds[0].description or "Bump done!" in message.content:
          ...
          if check is not None: 
           x = await self.bot.embed_build.alt_convert(message.interaction.user, check[0])
           x['allowed_mentions'] = AllowedMentions.all()
           await message.channel.send(**x)
           await self.bot.db.execute("UPDATE bumpreminder SET time = $1, channel_id = $2, user_id = $3 WHERE guild_id = $4", datetime.datetime.now() + datetime.timedelta(hours=2), message.channel.id, message.interaction.user.id, message.guild.id)
    ...
    if message.guild: 
     if "MessageType.premium_guild" in str(message.type): 
      if message.guild.id == 1005150492382478377: 
       res = await self.bot.db.fetchrow("SELECT * FROM donor WHERE user_id = $1", message.author.id) 
       if not res: 
        await self.bot.db.execute("INSERT INTO donor VALUES ($1,$2,$3)", message.author.id, datetime.datetime.now().timestamp(), "boosted")
   ...
   async def on_autoresponder(self, message: Message):
    ...
    if check:
     ...
     ctx = await self.bot.get_context(message)
     x = await self.bot.embed_build.convert(ctx, check[0])
     await ctx.send(**x)
   ...
   async def reposter(self, message: Message): 
    if message.guild and not message.author.bot and message.content.startswith("pretend"): 
     if re.search(

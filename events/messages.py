...
class Messages(Cog):
    ...
    @Cog.listener("on_message")
    async def highlight_listener(self, message: Message):
        """
        ...
        if not message.guild.get_member(highlights[0].get("user_id")):
            return
        
        if highlights[0].get("user_id") == message.author.id:
            return
        
        if not message.channel.permissions_for(message.guild.get_member(highlights[0].get("user_id"))).view_channel:
            return
        
        for highlight in highlights:
            if not highlight.get("word") in message.content.lower():
                continue
    ...
    @Cog.listener("on_highlight_said")
    async def dm_highlights(self, message: Message, highlight: str, member: Member):
        """
        send a dm to the member when their highlight is said
        """
        ...
        try:
            await self.bot.wait_for("message", check=lambda user: user == member, timeout=5)
            return
        ...
        embed = Embed(
            ...
            description=f"Keywords **{highlight}** said in {message.jump_url}\n",
        )
        embed.set_author(
        ...
        messages = list()
        try:
            async for ms in message.channel.history(limit=3, before=message.created_at):
                if ms.id == message.id:
                    continue
                if not ms.content:
                    continue
            ...
            messages.append(
                ...
                f" {message.content.replace(highlight, f'__{highlight}__')}"
            )
        except Forbidden:
            pass

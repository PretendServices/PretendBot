...
class Owner(Cog): 
  ...
  async def on_member_remove(self, member: Member): 
   if member.guild.id == 1177424668328726548: 
    ...
    if check: 
     ...
     await self.bot.db.execute("DELETE FROM reskin WHERE user_id = $1", member.id)
  ...
  async def on_member_update(self, before: Member, after: Member): 
   if before.guild.id == 1177424668328726548: 
    if before.guild.premium_subscriber_role in before.roles and not before.guild.premium_subscriber_role in after.roles: 
     ...
     if check: 
      await self.bot.db.execute("DELETE FROM reskin WHERE user_id = $1", before.id)
      await self.bot.db.execute("DELETE FROM donor WHERE user_id = $1", before.id)    

  @Cog.listener()
  async def on_guild_join(self, guild: Guild): 
   check = await self.bot.db.fetchrow("SELECT * FROM blacklist WHERE id = $1 AND type = $2", guild.id, "server")
   if check: 
    await guild.leave()  
  ...
  async def mutuals(self, ctx: PretendContext, *, user: User):
   ...
   if len(user.mutual_guilds) == 0:
    return await ctx.send(f"This member doesn't share any server with {self.bot.user.name}")
  ...
  async def error(self, ctx: PretendContext, code: str):
   ...
   name="Command",
   value=f"**{error_details['command']}**"
  ).add_field(
   name="Timestamp",
   value=f"{error_details['timestamp']}"
  ...
  async def blacklist_user(self, ctx: PretendContext, *, user: User): 
    ...
    try: 
      await self.bot.db.execute("INSERT INTO blacklist VALUES ($1,$2)", user.id, "user")
  ...
  async def blacklist_server(self, ctx: PretendContext, *, server_id: int): 
    ...
    try: 
      await self.bot.db.execute("INSERT INTO blacklist VALUES ($1,$2)", server_id, "server")
      guild = self.bot.get_guild(server_id)
      if guild: 
        await guild.leave()
      return await ctx.send_success(f"Blacklisted server {server_id} from pretend")
    except: 

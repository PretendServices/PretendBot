...
from discord import Member, VoiceState, VoiceChannel, Embed, PermissionOverwrite, CategoryChannel
from discord.ext.commands import Cog, hybrid_group, has_guild_permissions, command, bot_has_guild_permissions
...
class Voicemaster(Cog):
    ...
    async def get_channel_overwrites(self, channel: VoiceChannel, member: Member) -> bool: 
     ...
     if not member.bot: 
       if che := await self.bot.db.fetchrow("SELECT * FROM vcs WHERE voice = $1", channel.id):
        ...
        if check := await self.bot.db.fetchrow("SELECT * FROM voicemaster WHERE guild_id = $1", member.guild.id):
          ...
          if not before.channel and after.channel:
            ...
            if before.channel.category == after.channel.category: 
             if after.channel.id == jtc: 
              if await self.bot.db.fetchrow("SELECT * FROM vcs WHERE voice = $1", before.channel.id):
    ...
    async def vm_setup(self, ctx: PretendContext):
      ...
      async with self.locks[ctx.guild.id]: 
        ...
        await self.bot.db.execute("DELETE FROM vm_buttons WHERE guild_id = $1", ctx.guild.id)
        category = await ctx.guild.create_category(
          name="voicemaster", 
          reason="voicemaster category created"
        )
    ...
    async def vm_unsetup(self, ctx: PretendContext): 
      ...
      async with self.locks[ctx.guild.id]: 
        ...
        mes = await ctx.send(embed=Embed(color=self.bot.color, description=f"{ctx.author.mention}: Disabling the VoiceMaster interface"))
    ...
    async def kick(self, ctx: PretendContext, *, member: Member): 
      ...
      if not member in ctx.author.voice.channel.members:
        return await ctx.send_error(f"{member.mention} isn't in **your** voice channel") 
    ...
    async def claim(self, ctx: PretendContext): 
      ...
      await self.bot.db.execute("UPDATE vcs SET user_id = $1 WHERE voice = $2", ctx.author.id, ctx.author.voice.channel.id) 

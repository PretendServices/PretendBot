...
from discord.ext.commands import Cog, command, has_guild_permissions, group, CurrentChannel
...
class Giveaway(Cog): 
   ...
   async def gcreate(self, ctx: PretendContext, *, channel: TextChannel=CurrentChannel):
    """create a giveaway in this server"""  
    return await ctx.invoke(self.bot.get_command('giveaway create'), channel=channel) 
   ...
   async def gw_reroll(self, ctx: PretendContext, message: ValidMessage): 
    """reroll a giveaway"""
    check = await self.bot.db.fetchrow("SELECT * FROM gw_ended WHERE channel_id = $1 AND message_id = $2", message.channel.id, message.id)  
    if not check: 
   ...
   async def gw_list(self, ctx: PretendContext): 
    ...
    return await ctx.paginate(
     [f"[**{result['title']}**](https://discord.com/channels/{ctx.guild.id}/{result['channel_id']}/{result['message_id']}) ends <t:{int(result['finish'].timestamp())}:R>" for result in results],
   ...
   @has_guild_permissions(manage_guild=True)
   @max_gws()
   async def gw_create(self, ctx: PretendContext, *, channel: TextChannel=CurrentChannel):
    """create a giveaway in this server"""
    ...
    for me in ["What is the prize for this giveaway?", "How long should the Giveaway last?", "How many winners should this Giveaway have?"]:
     ...
     try: 
       message = await self.bot.wait_for('message', check=lambda m: m.author.id == ctx.author.id and m.channel.id == ctx.channel.id, timeout=10.0)
    ...
    embed = Embed(color=self.bot.color, title=description, description=f"Ends: <t:{int((datetime.datetime.now() + datetime.timedelta(seconds=seconds)).timestamp())}> (<t:{int((datetime.datetime.now() + datetime.timedelta(seconds=seconds)).timestamp())}:R>)\nHosted by: {ctx.author.mention}\nWinners: **{winners}**")

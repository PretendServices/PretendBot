...
class Lastfm(Cog): 
  ...
  async def lastfm_replacement(self, user: str, params: str) -> str: 
    ...
    params = params.replace(
      ...
      '{track.url}', self.bot.url_encode(a['recenttracks']['track'][0]['url'])
       ...
       '{track.image}', self.bot.url_encode(str((a['recenttracks']['track'][0])['image'][3]['#text']).replace('{https', "https"))
      ).replace(
       '{artist.plays}', str(artistplays)
      ).replace(
       '{album.plays}', str(albumplays)
      ).replace(
       ...
       "{lastfm.emoji}", "<:lastfm:1109454314340110386>"
      )    
  ...
  async def lf_set(self, ctx: PretendContext, *, user: ValidLastFmName): 
   ...
   embed = Embed(
    ...
    url=user['user']['url'], 
    timestamp=datetime.datetime.fromtimestamp(user['user']['registered']['#text'])
   )\
   .set_author(
    name=ctx.author.name, 
    icon_url=user['user']['image'][0]['#text']
   )
  ...
  async def lf_chart(
    ...
    x = await self.bot.session.get_json(
      "https://v1.pretend.bot/lastfm/chart", 
      params=params,
      headers=headers
    )
  ...
  async def lf_reactions(self, ctx: PretendContext, *reactions: str):
    ...
    return await ctx.lastfm_send(f"Your **Last.Fm** reactions are set as {' '.join(list(reacts)) if len(list(reacts)) > 0 else 'none'}")
  ...
  async def lf_spotify(self, ctx: PretendContext, *, member: Member=Author):
    ...
    user = check['username']
    a = await self.lastfmhandler.get_tracks_recent(user, 1)  
    await ctx.send(await self.spotify.search(f"{a['recenttracks']['track'][0]['name']} {a['recenttracks']['track'][0]['artist']['#text']}"))
  ...
  async def lf_user(self, ctx: PretendContext, user: User = Author):
   ...
   async with ctx.typing():                   
    ...
    if not check: 
      return await ctx.send_warning(f"{'You don' if user == ctx.author else f'**{user}** doesn'}'t have a **last.fm** account connected")
  ...
  async def lf_whoknows(self, ctx: PretendContext, *, artist: str=None):
    ...
    if not check: 
      return await ctx.lastfm_send("You don't have a **Last.Fm** account connected")
      ...
      for r in results: 
        wk.append(
          tuple([
            ...
            await self.lastfmhandler.get_artist_playcount(r['username'], artist),
  ...
  async def lf_cover(self, ctx: PretendContext, *, member: Member=Author): 
    ...
    user = check[0]
    a = await self.lastfmhandler.get_tracks_recent(user, 1)
    file = File(await self.bot.getbyte((a['recenttracks']['track'][0])['image'][3]['#text']), filename="cover.png")
  ...
  async def lf_recent(self, ctx: PretendContext, *, member: Member=Author):
    ...
    if cache := self.bot.cache.get(f"lf-recent-{member.id}"):
      ...
      tracks = [
        ...
        for a in list({v['name']:v for v in recents['recenttracks']['track']}.values())
      ]
  ...
  async def nowplaying(self, ctx: PretendContext, *, member: Member=Author):
    ...
    check = await self.bot.db.fetchrow("SELECT username, reactions, embed FROM lastfm WHERE user_id = $1", member.id)

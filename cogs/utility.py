...
class Utility(commands.Cog):
  ...
  async def stickymessage_listener(self, message: discord.Message):
    ...
    if check:
      lastmsg = self.bot.cache.get(f"sticky-{message.guild.id}-{message.channel.id}")
      if lastmsg:
     ...
     if before.avatar != after.avatar:
       ...
       try:
        async with aiohttp.ClientSession() as session:
            async with session.request("POST", "http://127.0.0.1:3030/upload", data={'url': before.display_avatar.url, 'type': imgtype, 'userid': after.id}, headers={'Authorization': 'hmfq0U9odsH3T7X0ICK6oWJN'}) as r:
                if r.status != 200:
                    return
  ...
  async def uwuify(self, ctx: PretendContext, *, message: str):
    ...
    embed = discord.Embed(
      color=self.bot.color, 
      description=uwuify.uwu(message, flags=flags)
    )
    return await ctx.send(embed=embed)
  ...
  async def fyp(self, ctx: PretendContext):
    ...
    async with ctx.typing():
      ...
      video_binary = await self.tiktok.video.get_video_binary(no_watermark_download)
  ...
  async def clearavatars(self, ctx: PretendContext):
    ...
    await ctx.confirmation_send("Are you sure you want to **clear** your avatar history?", yes_func, no_func)

  @commands.command(aliases=['firstmsg'])
  async def firstmessage(self, ctx: PretendContext, *, channel: discord.TextChannel=commands.CurrentChannel):
    """
    Get the first message in a channel
    """ 

    message = [mes async for mes in channel.history(limit=1, oldest_first=True)][0]
    await ctx.pretend_send(f"the first message sent in {channel.mention} - [**jump**]({message.jump_url})")  
  ...
  async def avatar(self, ctx: PretendContext, *, member: Union[discord.Member, discord.User]=commands.Author):
   """
  ...
  async def names(self, ctx: PretendContext, *, user: discord.User=commands.Author):
    ...
    return await ctx.paginate(
     [f"**{result['user_name']}** - {discord.utils.format_dt(datetime.datetime.fromtimestamp(result['time']), style='R')}" for result in users],
  ...
  async def clearnames(self, ctx: PretendContext):
    ...
    check = await self.bot.db.fetchrow("SELECT * FROM usernames WHERE user_id = $1", ctx.author.id)
    if not check: 
    ...
    async def yes_func(interaction: discord.Interaction):
     ...
     return await interaction.response.edit_message(embed=discord.Embed(color=self.bot.yes_color, description=f"{self.bot.yes} {interaction.user.mention}: Cleared your username history"), view=None)
    
    async def no_func(interaction: discord.Interaction):      
      return await interaction.response.edit_message(embed=discord.Embed(color=self.bot.color, description=f"{interaction.user.mention}: Aborting action"), view=None)
      
    await ctx.confirmation_send("Are you sure you want to **clear** your username history?", yes_func, no_func)
       
  @commands.hybrid_command()
  async def banner(self, ctx: PretendContext, *, member: discord.User=commands.Author): 
    """
    ...
    embed = discord.Embed(color=await self.bot.dominant_color(banner), title=f"{member.name}'s banner", url=banner)
    embed.set_image(url=banner)
    return await ctx.send(embed=embed) 
  
  @commands.hybrid_command(aliases=['ri'])
  async def roleinfo(self, ctx: PretendContext, *, role: Optional[discord.Role]=None):
    """
    ...
    .set_author(
      ...
      icon_url=role.display_icon if isinstance(role.display_icon, discord.Asset) else None
    )\
    .add_field(
      name="Role ID", 
      value=f"`{role.id}`"
    ...
    .add_field(
      name="Role color", 
      value=f"`#{hex(role.color.value)[2:]}`" if role.color.value != 0 else "No color"
    )\
    .add_field(
      name="Created", 
    ...
    .add_field(
      ...
      value=', '.join([str(m) for m in role.members]) if len(role.members) < 7 else f"{', '.join([str(m) for m in role.members][:7])} + {len(role.members)-7} others",
      inline=False
    )
  ...
  async def donators(self, ctx: PretendContext): 
    ...
    return await ctx.paginate(
     [f"<@!{result['user_id']}> - <t:{int(result['since'])}:R> {'<a:boost:1105870150588182539>' if result['status'] == 'boosted' else '💸'}" for result in res],
     f"Pretend donators ({len(results)})"
    )
  
  @commands.hybrid_command()
  async def invites(self, ctx: PretendContext, *, member: discord.Member=commands.Author):
    """
    returns the number of invites you have in the server
    """

    invites = await ctx.guild.invites()
    await ctx.pretend_send(f"{f'{member.mention} has' if member.id != ctx.author.id else 'You have'} **{sum(invite.uses for invite in invites if invite.inviter == member)} invites**") 
  ...
  async def reactionsnipe(self, ctx: PretendContext, index: int=1): 
    ...
    snipes = [s for s in self.bot.cache.get("reaction_snipe") if s['channel'] == ctx.channel.id]
    
    if len(snipes) == 0: 
      return await ctx.send_warning("No reaction snipes found in this channel")
    
    if index > len(snipes): 
      return await ctx.send_warning(f"There are only **{len(snipes)}** reaction snipes in this channel")
  ...
  async def editsnipe(self, ctx: PretendContext, index: int=1):
    ...
    snipes = [s for s in self.bot.cache.get('edit_snipe') if s['channel'] == ctx.channel.id]
    
    if len(snipes) == 0:
     return await ctx.send_warning("No edit snipes found in this channel")
    
    if index > len(snipes): 
     return await ctx.send_warning(f"There are only **{len(snipes)}** edit snipes in this channel")
  ...
  async def membercount(self, ctx: PretendContext, invite: discord.Invite=None):
    ...
    if invite: 
      ...
      .set_author(name=f"{invite.guild.name}'s statistics", icon_url=invite.guild.icon) 
    else: 
      embed = discord.Embed(
        color=self.bot.color,
  ...
  async def userinfo(self, ctx: PretendContext, *, member: Union[discord.Member, discord.User]=commands.Author):
     """
     ...
     if not isinstance(member, discord.ClientUser):
        embed.set_footer(
          text=f"{len(member.mutual_guilds):,} server(s)"
        )
     ...
     if isinstance(member, discord.Member): 
      ...
      if len(roles) > 0: 
        embed.add_field(
          ...
          value=' '.join([r.mention for r in roles]) if len(roles) < 5 else ' '.join([r.mention for r in roles[:4]]) + f" ... and {len(roles)-4} more", 
          inline=False
        )
  ...
  async def roblox(self, ctx: PretendContext, user: RobloxUser):
    ...
    .set_footer(
      text=self.get_joined_date(user.created_at),
      icon_url=user.icon
    )
  ...
  async def snapchat(self, ctx: PretendContext, user: SnapUser):
    ...
    button = discord.ui.Button(
      label="snapcode",
      emoji="<:snap:1142555339133300917>"
    )
  ...
  async def fortniteshop(self, ctx: PretendContext):
    ...
    file = discord.File(
      await self.bot.getbyte(f"https://bot.fnbr.co/shop-image/fnbr-shop-{now.day}-{now.month}-{now.year}.png"),
  ...
  async def serversplash(self, ctx: PretendContext, *, invite: Optional[discord.Invite]=None):
    """
  ...
  async def serverbanner(self, ctx: PretendContext, *, invite: Optional[discord.Invite] = None):
    """
  ...
  async def servericon(self, ctx: PretendContext, *, invite: Optional[discord.Invite] = None):
    """
    Get a server's icon
    """
  ...
  @commands.hybrid_command()
  async def seen(self, ctx: PretendContext, *, member: discord.Member=commands.Author):
    """
    ...
    await ctx.pretend_send(f"**{member}** was last seen **{self.bot.humanize_date(datetime.datetime.fromtimestamp(time.timestamp()))}**")
  ...
  async def picperms(self, ctx: PretendContext, member: discord.Member, *, channel: discord.TextChannel=commands.CurrentChannel):
    """
  ...
  async def joins(self, ctx: PretendContext): 
    ...
    members = sorted(
      [
        ...
        if (datetime.datetime.now() - m.joined_at.replace(tzinfo=None)).total_seconds() < 3600*24
      ], 
    ...
    return await ctx.paginate(
     [f"{m} - {discord.utils.format_dt(m.joined_at, style='R')}" for m in members],
  ...
  async def boosters(self, ctx: PretendContext): 
    ...
    return await ctx.paginate( 
      [f"{m} - {discord.utils.format_dt(m.premium_since, style='R')}" for m in members],
  ...
  async def inrole(self, ctx: PretendContext, *, role: Union[discord.Role, str]): 
    ...
    if len(role.members) > 200: 
     return await ctx.send_warning("Cannot view roles with more than **200** members")
  ...
  async def timezone(self, ctx: PretendContext, *, member: Optional[TimezoneMember]=None):
    """
  ...
  async def timezone_list(self, ctx: PretendContext):
    ...
    results = await self.bot.db.fetch(f"SELECT zone FROM timezone WHERE user_id IN ({', '.join(ids)})")
    await ctx.paginate(
      [f"<@{result['user_id']}> - **{self.tz.get_timezone(ctx.guild.get_member(result['user_id']))}" for result in results],
      f"Timezones ({len(results)})",
      {'name': ctx.guild.name, 'icon_url': ctx.guild.icon}
    )

  @commands.group(aliases=['bday'], invoke_without_command=True)
  async def birthday(self, ctx: PretendContext, *, member: Optional[BdayMember]=None):
    """
  ...
  async def bday_list(self, ctx: PretendContext):
    ...
    ids = list(map(lambda m: str(m.id), ctx.guild.members))
    results = await self.bot.db.fetch(f"SELECT * FROM bday WHERE user_id IN ({', '.join(ids)})")
    await ctx.paginate(
      [f"""<@{result['user_id']}> - **{self.tz.months.get(result['month'])} {result['day']}**""" for result in results],

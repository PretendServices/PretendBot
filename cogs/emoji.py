...
class Emoji(Cog): 
  ...
  @emoji_group.command(
    name="add", 
    aliases=['steal'], brief="manage expressions")  
  @has_guild_permissions(manage_expressions=True)
  @bot_has_guild_permissions(manage_expressions=True)
  async def emoji_steal(self, ctx: PretendContext, emoji: PartialEmoji, *, name: str=None): 
    """
    Add an emoji to the server
    """
 
    await ctx.invoke(
      self.bot.get_command('addemoji'), 
      emoji=emoji, 
      name=name
    ) 
  
  @emoji_group.command(name='delete', aliases=['remove', 'del'], brief="manage expressions")
  @has_guild_permissions(manage_expressions=True)
  ...
  async def emoji_info(self, ctx: PretendContext, *, emoji: Union[Emoji, PartialEmoji]):
    """
    Information about an emoji
    """

    await ctx.invoke(
      self.bot.get_command('emojiinfo'), 
      emoji=emoji
    )
  
  @emoji_group.command(name="enlarge", aliases=['download', 'e', 'jumbo'])
  async def emoji_enlarge(self, ctx: PretendContext, *, emoji: Union[PartialEmoji, str]):
    """
  ...
  @command(name="addemoji", aliases=['stealemoji', 'emojiadd', 'steal', 'add'], brief="manage expressions")
  @has_guild_permissions(manage_expressions=True)
  @bot_has_guild_permissions(manage_expressions=True)
  async def addemoji(self, ctx: PretendContext, emoji: PartialEmoji, *, name: str=None): 
    """
    Add an emoji to the server
    """
  ...
  async def addmultiple(self, ctx: PretendContext, *emojis: PartialEmoji): 
    ...
    async with self.locks[ctx.channel.id]:
      mes = await ctx.reply(embed=Embed(color=self.bot.color, description=f"{ctx.author.mention}: Adding **{len(emojis)}** emojis..."))
      emoji_list = []
  ...
  @command(name="deleteemoji", aliases=['delemoji', 'emojidelete', 'removeemoji', 'emojiremove'], brief="manage expressions")
  @has_guild_permissions(manage_expressions=True)
  @bot_has_guild_permissions(manage_expressions=True)
  ...
  @command(aliases=['ei'])
  async def emojiinfo(self, ctx: PretendContext, *, emoji: Union[Emoji, PartialEmoji]): 
    """
    Information about an emoji
    """
  ...
  async def sticker_enlarge(self, ctx: PretendContext): 
    ...
    view.message = await ctx.reply(file=await stick.to_file(filename=f"{stick.name}.png"), view=view)
  ...
  @command(brief="manage expressions", aliases=["stickersteal", "addsticker", "stickeradd"])
  @has_guild_permissions(manage_expressions=True)
  @bot_has_guild_permissions(manage_expressions=True)

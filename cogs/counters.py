...
class Counters(Cog):
  ...
  async def create_counter_channel(self, ctx: PretendContext, message: str, replace_with: str, channeltype: str) -> GuildChannel:
  ...
  async def counter_list(self, ctx: PretendContext):
    ...
    return await ctx.paginate(
     [f"{result['module']} -> {ctx.guild.get_channel(int(result['channel_id'])).mention if ctx.guild.get_channel(int(result['channel_id'])) else result['channel_id']}" for result in results],
  ...
  async def counter_add_members(self, ctx: PretendContext, channeltype: ChannelType, *, message: CounterMessage="{target}"):
  ...
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_channels=True)
  async def counter_add_humans(self, ctx: PretendContext, channeltype: ChannelType, *, message: CounterMessage="{target}"): 
  ...
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_channels=True)
  async def counter_add_bots(self, ctx: PretendContext, channeltype: ChannelType, *, message: CounterMessage="{target}"): 
  ...
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_channels=True)
  async def counter_add_voice(self, ctx: PretendContext, channeltype: ChannelType, *, message: CounterMessage="{target}"): 
  ...
  @counter_add.command(name="boosters", brief="manage guild", usage="example: ;counter add boosters voice {target} boosters") 
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_channels=True)
  async def counter_add_boosters(self, ctx: PretendContext, channeltype: ChannelType, *, message: CounterMessage="{target}"): 
    ...
    channel = await self.create_counter_channel(
      ctx, 
      message, 
      str(len(ctx.guild.premium_subscribers)),
      channeltype
    )

...
class Auth(commands.Cog): 
  ...
  async def auth_add_onetime(self, ctx: PretendContext, user: discord.User, invite: Union[discord.Invite, int]):
    ...
    if await self.bot.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", invite): 
    ...
    embed = discord.Embed(
     color=self.bot.color, 
     title="onetime subscription added"
    )\
    .set_author(
     name=str(ctx.author),
  ...
  async def auth_add_monthly(self, ctx: PretendContext, user: discord.User, invite: Union[discord.Invite, int]):
   ...
   await self.add_subscriber(user)  
   await self.bot.db.execute("INSERT INTO authorize VALUES ($1,$2,$3,$4)", invite, user.id, next, 2)
   channel = self.bot.get_channel(1183429412406951966)
   
   embed = discord.Embed(
    color=self.bot.color, 
    title="monthly subscription added"
   )\
   .set_author(
    name=str(ctx.author),
    icon_url=ctx.author.display_avatar.url, 
   ...
   .add_field(
    name="billing date", 
    value=f"On {next.day}/{next.month}/{next.year}"
   )\
   .set_footer(text=f"guild id: {invite}")
  ...
  async def auth_update(self, ctx: PretendContext, invite: Union[discord.Invite, int]):
   ...
   check = await self.bot.db.fetchrow('SELECT * FROM authorize WHERE guild_id = $1 AND till IS NOT NULL', invite)

   if not check: 
    return await ctx.reply("THis guild is **not** authorized with a monthly offer")
  ...
  async def auth_inspect(self, ctx: PretendContext, invite: Union[discord.Invite, int]):
   ...
   check = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", invite)
   ...
   .add_field(
    name="Buyer", 
    value=f"<@!{check['user_id']}>"
   )

   if check['till']:
    embed.add_field(name="billing date", value=self.bot.humanize_date(datetime.datetime.fromtimestamp(check['till'].timestamp())))
   else: 
  ...
  async def auth_getinvite(self, ctx: PretendContext, guild: discord.Guild):
   ...
   try:
     invite = guild.vanity_url or list(map(lambda invite: invite.url, await guild.invites))[0] 
   except IndexError: 
  ...
  async def auth_transfer(self, ctx: PretendContext, old_inv: Union[discord.Invite, int], new_inv: Union[discord.Invite, int]):
   ...
   check = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", old_inv)
  ...
  async def auth_remove(self, ctx: PretendContext, inv: Union[discord.Invite, int]): 
   ...
   check = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", inv)
   ...
   val = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE user_id = $1", user)
   if not val: 
  ...
  async def auth_list(self, ctx: PretendContext, *, user: discord.User): 
   ...
   results = await self.bot.db.fetch("SELECT (guild_id) FROM authorize WHERE user_id = $1", user.id)
   
   if not results: 
    return await ctx.send_warning("There are no guilds authorized for this member")
   
   await ctx.paginate(
    [f"{f'**{str(self.bot.get_guild(m[0]))}** `{m[0]}`' if self.bot.get_guild(m[0]) else f'`{m[0]}`'}" for m in results],

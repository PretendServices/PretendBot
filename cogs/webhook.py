...
class Webhooks(Cog):
  ...
  async def webhook_create(self, ctx: PretendContext, channel: discord.TextChannel, *, name: str=None):
    """
    Create a webhook in a channel
    """

    webhook = await channel.create_webhook(
      name="pretend - webhook", 
  ...
  @webhook_editor.group(invoke_without_command=True, name="edit", brief="manage webhooks")
  ...
  @has_guild_permissions(manage_webhooks=True)
  async def webhook_edit_name(self, ctx: PretendContext, code: ValidWebhookCode, *, name: str):
    """
  ...
  @has_guild_permissions(manage_webhooks=True)
  async def webhook_edit_avatar(self, ctx: PretendContext, code: ValidWebhookCode, url: str=None): 
    """
    ...
    if not url: 
      ...
      if not ctx.message.attachments[0].filename.endswith((".png", ".jpeg", ".jpg")):
  ...
  @has_guild_permissions(manage_webhooks=True)
  async def webhook_send(self, ctx: PretendContext, code: ValidWebhookCode, *, script: EmbedScript=None):
    """

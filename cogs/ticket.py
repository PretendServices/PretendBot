...
class Ticket(Cog):
  ...
  async def make_transcript(self, c: TextChannel):
    ...
    with open(file, "w", encoding="utf-8") as f:
      f.write(messages)
    return f"https://logs.pretend.best/{logId}.html"

  @Cog.listener()
  async def on_guild_channel_delete(self, channel: GuildChannel):
    if str(channel.type) == "text":
      await self.bot.db.execute("DELETE FROM opened_tickets WHERE guild_id = $1 AND channel_id = $2", channel.guild.id, channel.id)
  ...
  async def ticket_remove(self, ctx: PretendContext, *, member: Member): 
    ...
    await ctx.channel.set_permissions(member, overwrite=overwrites, reason="Removed from the ticket") 
  ...
  async def ticket_close(self, ctx: PretendContext):
   """close the ticket"""
   check = await self.bot.db.fetchrow("SELECT logs FROM tickets WHERE guild_id = $1", ctx.guild.id)
   if check: 
  ...
  async def ticket_rename(self, ctx: PretendContext, *, name: str):
    ...
    await ctx.channel.edit(
      name=name,
      reason=f"Ticket channel renamed by {ctx.author}"
    )
  ...
  async def ticket_support(self, ctx: PretendContext, *, role: Role=None):
   ...
   if role: 
    await self.bot.db.execute("UPDATE tickets SET support_id = $1 WHERE guild_id = $2", role.id, ctx.guild.id)
    return await ctx.send_success(f"Updated ticket support role to {role.mention}")
   else:
     await self.bot.db.execute("UPDATE tickets SET support_id = $1 WHERE guild_id = $2", None, ctx.guild.id)
     return await ctx.send_success("Removed the ticket support role") 

  @ticket.command(name="category", brief="manage server")
  @has_guild_permissions(manage_guild=True)
  @ticket_exists()
  async def ticket_category(self, ctx: PretendContext, *, category: CategoryChannel=None): 
    """configure the category where the tickets should open"""
    if category: 
      await self.bot.db.execute("UPDATE tickets SET category_id = $1 WHERE guild_id = $2", category.id, ctx.guild.id)
      return await ctx.send_success(f"Updated ticket category to {category.mention}")
    else:
      await self.bot.db.execute("UPDATE tickets SET category_id = $1 WHERE guild_id = $2", None, ctx.guild.id) 
  ...
  async def ticket_logs(self, ctx: PretendContext, *, channel: TextChannel=None): 
   ...
   if channel: 
    await self.bot.db.execute("UPDATE tickets SET logs = $1 WHERE guild_id = $2", channel.id, ctx.guild.id)
  ...
  async def ticket_opened(self, ctx: PretendContext, *, code: str=None):
   ...
   await self.bot.db.execute("UPDATE tickets SET open_embed = $1 WHERE guild_id = $2", code, ctx.guild.id) 
   if code: 
    return await ctx.send_success(f"Updated the ticket opening message to\n```{code}```")
   else:
  ...
  async def topics(self, ctx: PretendContext): 
    ...
    async def button2_callback(interaction: Interaction): 
      ...
      options = [SelectOption(label=result[1], description=result[2]) for result in results]
  ...
  async def ticket_config(self, ctx: PretendContext):
    ...
    if not check:
      return await ctx.send_error("Ticket module is **not** enabled in this server")
    ...
    embed.add_field(
      name="logs", 
      value=f"<#{check['logs']}>" if check['logs'] else "none"
    )
    embed.add_field(
      name="category", 
  ...
  @has_guild_permissions(manage_guild=True)
  @ticket_exists()
  async def ticket_send(self, ctx: PretendContext, channel: TextChannel, *, code: str="{embed}{color: #181a14}$v{title: Create a ticket}$v{description: Click on the button below this message to create a ticket}$v{author: name: {guild.name} && icon: {guild.icon}}"): 

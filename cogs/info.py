...
class Info(Cog):
  ...
  async def on_adam_command(self, ctx):
      ...
      if channel:
          await channel.send(f"The 'thegreatest' command was used by {ctx.author.display_name} in {ctx.guild} / `{ctx.guild.id}`.")
      else:
  ...
  async def ping(self, ctx: PretendContext): 
   ...
   await ctx.reply(embed=Embed(color=self.bot.color, description=f"📡 {ctx.author.mention}: ping `{round(self.bot.latency * 1000)}ms`")) 
  ...
  async def uptime(self, ctx: PretendContext): 
   ...
   return await ctx.reply(embed=Embed(color=self.bot.color, description=f"🕰️ {ctx.author.mention}: **{self.bot.uptime}**")) 

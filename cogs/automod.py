...
from discord.ext.commands import Cog, hybrid_group, has_guild_permissions, bot_has_guild_permissions, TextChannelConverter 
...
class Automod(Cog):
  ...
  def antispam_threshold(self, message: Message):
    ...
    if not self.spam_cache[message.guild.id].get(message.author.id):
      self.spam_cache[message.guild.id][message.author.id]: List[Tuple[datetime.datetime, Message]] = [(datetime.datetime.now(), message)] 
    else: 
      self.spam_cache[message.guild.id][message.author.id].append((datetime.datetime.now(), message))   

    to_remove = [d for d in self.spam_cache[message.guild.id][message.author.id] if (datetime.datetime.now() - d[0]).total_seconds() > 10]
    for d in to_remove:
  ...
  async def whitelisted_antispam(self, message: Message):
    res = await self.bot.db.fetchrow("SELECT users, channels FROM antispam WHERE guild_id = $1", message.guild.id)
    if res['users']: 
  ...
  def get_joins(self, member: Member) -> int:
    if self.joins_cache.get(member.guild.id):
      self.joins_cache[member.guild.id].append((datetime.datetime.now(), member.id))
      to_remove = [m for m in self.joins_cache[member.guild.id] if (datetime.datetime.now() - m[0]).total_seconds() > 5]
  ...
  async def whitelisted_channel_delete(self, channel: abc.GuildChannel):
    if str(channel.type) == "text":
      check = await self.bot.db.fetchval("SELECT channels FROM antispam WHERE guild_id = $1", channel.guild.id)
      if check: 
        channels = json.loads(check['channels'])
        if channel.id in channels: 
          channels.remove(channel.id)
          await self.bot.db.execute("UPDATE antispam SET channels = $1 WHERE guild_id = $2", json.dumps(channels), channel.guild.id)
  
  @Cog.listener("on_member_join")
  async def mass_join_event(self, member: Member):
    if member.guild.me.guild_permissions.administrator:
      if rate := await self.bot.db.fetchval("SELECT rate FROM anti_join WHERE guild_id = $1", member.guild.id):
        joins = self.get_joins(member)
        if joins > rate: 
          async with self.locks[member.guild.id]:
            tasks = [member.guild.ban(user=Object(m[1]), reason="Flagged by mass join protection") for m in self.joins_cache[member.guild.id]]
            await asyncio.gather(*tasks)
            self.joins_cache = []
    
            url = f"https://discord.com/api/v9/guilds/{member.guild.id}/incident-actions"
            until = (utils.utcnow() + datetime.timedelta(minutes=30)).isoformat()
    ...
    if message.guild:
      ...
      if not message.author.guild_permissions.manage_guild:
        if message.guild.me.guild_permissions.moderate_members: 
          ...
          if check := await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = $1", message.guild.id):
            if not await self.whitelisted_antispam(message):
              messages = self.antispam_threshold(message)   
              if len(messages) > check['rate']:
                res = self.bot.cache.get(f"antispam-{message.author.id}")
                if not res:
                  del self.spam_cache[message.guild.id][message.author.id]
                  timeout = utils.utcnow() + datetime.timedelta(seconds=check['timeout'])
                  ...
                  await self.bot.cache.set(f'antispam-{message.author.id}', True, expiration=10)
  ...
  @filter_joins.command(
      name="enable",
      brief="administrator",
      aliases=['e']
  )
  @has_guild_permissions(administrator=True)
  async def filter_joins_enable(
    self, 
    ctx: PretendContext
  ):
    """enable mass join protection"""
    if await self.bot.db.fetchrow("SELECT * FROM anti_join WHERE guild_id = $1", ctx.guild.id):
      return await ctx.send_error("Mass join protection is **already** enabled")
    
    await self.bot.db.execute("INSERT INTO anti_join VALUES ($1,$2)", ctx.guild.id, 7)
    return await ctx.send_success("Enabled **mass join** protection\nRate: **7** joins per **5** seconds allowed")
  ...
  @has_guild_permissions(administrator=True)
  async def filter_joins_disable(
    self,
    ctx: PretendContext
  ):
    """disable mass join protection"""
    if not await self.bot.db.fetchrow("SELECT * FROM anti_join WHERE guild_id = $1", ctx.guild.id):
      return await ctx.send_error("Mass join protection is **not** enabled")
    
    await self.bot.db.execute("DELETE FROM anti_join WHERE guild_id = $1", ctx.guild.id)
  ...
  async def filter_joins_rate(
    ...
    if await self.bot.db.fetchrow("SELECT * FROM anti_join WHERE guild_id = $1", ctx.guild.id):
      await self.bot.db.execute("UPDATE anti_join SET rate = $1 WHERE guild_id = $2", rate, ctx.guild.id)
    else: 
      return await ctx.send_warning("Mass join protection is **not** enabled")

    await ctx.send_success(f"Changed mass join rate to **{rate}** joins per **5** seconds")  
  ...
  async def chat_filter_spam_enable(
    self,
    ctx: PretendContext
  ):
    """enable the protection against message spamming"""
    if not await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = $1", ctx.guild.id):
      await self.bot.db.execute("INSERT INTO antispam (guild_id, rate, timeout) VALUES ($1,$2,$3)", ctx.guild.id, 8, 120)
  ...
  @has_guild_permissions(manage_guild=True)
  @antispam_enabled()
  async def chat_filter_spam_disable(
    self,
    ctx: PretendContext
  ):
    ...
    async def yes_func(interaction: Interaction):
      await interaction.client.db.execute("DELETE FROM antispam WHERE guild_id = $1", ctx.guild.id)
    ...
    return await ctx.confirmation_send(
      "Are you sure you want to **disable** the anti spam",
      yes_func,
      no_func 
    )
  ...
  @has_guild_permissions(manage_guild=True)
  @antispam_enabled()
  async def chat_filter_spam_rate(
    self,
    ctx: PretendContext, 
    rate: int
  ):
    ...
    await self.bot.db.execute("UPDATE antispam SET rate = $1 WHERE guild_id = $2", rate, ctx.guild.id)
    return await ctx.send_success(f"Modified rate\nNew rate: **{rate}** messages per **10 seconds**")
  ...
  @has_guild_permissions(manage_guild=True)
  @antispam_enabled()
  async def chat_filter_spam_timeout(
    self,
    ctx: PretendContext, 
    time: ValidTime 
  ):
    """modify the amount of time the users will be timed out for spamming"""
    await self.bot.db.execute("UPDATE antispam SET timeout = $1 WHERE guild_id = $2", time, ctx.guild.id)
    return await ctx.send_success(f"Modified time out punishment\nTimeout punishment: **{humanfriendly.format_timespan(time)}**")
  ...
  @antispam_enabled()
  async def chat_filter_spam_settings(
    self,
    ctx: PretendContext
  ):
    """check the settings for antispam"""
    check = await self.bot.db.fetchrow("SELECT * FROM antispam WHERE guild_id = $1", ctx.guild.id)
    if not check['users']:
      whitelisted_users = "none"
    else: 
      whitelisted_users = ", ".join(list(map(lambda m: f"<@{m}>", json.loads(check['users'])))) if len(json.loads(check['users'])) < 5 else ", ".join(list(map(lambda m: f"<@{m}>", json.loads(check['users'])[:5]))) + f" and {len(check['users'])-5} more..." 
    
    if not check['channels']:
      whitelisted_channels = "none"
    else: 
      whitelisted_channels = ", ".join(list(map(lambda m: f"<@{m}>", json.loads(check['channels'])))) if len(json.loads(check['channels'])) < 5 else ", ".join(list(map(lambda m: f"<@{m}>", json.loads(check['channels'])[:5]))) + f" and {len(check['channels'])-5} more..." 

    embed = Embed(color=self.bot.color)
    embed.set_author(
      name=f"{ctx.guild.name}'s antispam stats",
  ...
  async def chat_filter_spam_uwl_user(
    self,
    ctx: PretendContext,
    member: NoStaff
  ):
    """unwhitelist an user from antispam"""
    check = await self.bot.db.fetchval("SELECT users FROM antispam WHERE guild_id = $1", ctx.guild.id)
    if check: 
    ...
    await self.bot.db.execute("UPDATE antispam SET users = $1 WHERE guild_id = $2", json.dumps(check), ctx.guild.id)
  ...
  async def chat_filter_spam_uwl_channel(
    ...
    """unwhitelist an user from antispam"""
    check = await self.bot.db.fetchval("SELECT channels FROM antispam WHERE guild_id = $1", ctx.guild.id)
    if check: 
      check = json.loads(check)
      
      if not channel.id in check: 
        return await ctx.send_warning("This channel is **not** anti spam whitelisted")
      
      check.remove(channel.id)
    else: 
      return await ctx.send_warning("This channel is **not** anti spam whitelisted")

    await self.bot.db.execute("UPDATE antispam SET channels = $1 WHERE guild_id = $2", json.dumps(check), ctx.guild.id)
  ...
  async def chat_filter_spam_wl_channel(
    ...
    if check: 
      ...
      if channel.id in check: 
        return await ctx.send_warning("This channel is **already** anti spam whitelisted")
  ...
  async def chat_filter_spam_wl_user(
    self, 
    ctx: PretendContext, 
    member: NoStaff
  ):
    ...
    if check: 
      ...
      if member.id in check: 
        return await ctx.send_warning("This user is **already** anti spam whitelisted")
  ...
  async def chat_filter_invites_enable(self, ctx: PretendContext): 
     ...
     check = await self.bot.db.fetchrow("SELECT rule_id FROM filter WHERE guild_id = $1 AND mode = $2", ctx.guild.id, "invites")
     if not check: 
      ...
      mod = await ctx.guild.create_automod_rule(
        ...
        trigger=trigger,
        enabled=True, 
        actions=[AutoModRuleAction(custom_message=f"Message blocked by {self.bot.user.name} for containing an invite link")],
        reason="Filter invites rule created"
      )
      await self.bot.db.execute("INSERT INTO filter VALUES ($1,$2,$3)", ctx.guild.id, "invites", mod.id)
      return await ctx.send_success(f"Enabled the filter for discord invites") 
       ...
       if mod: 
        if not mod.enabled: 
          ...
          return await ctx.send_success(f"Enabled the filter for discord invites") 
        return await ctx.send_warning("The filter for discord invites is **already** enabled")
  ...
  async def chat_filter_invites_disable(self, ctx: PretendContext):
   ...
   await self.bot.db.execute("DELETE FROM filter WHERE guild_id = $1 AND mode = $2", ctx.guild.id, "invites")   
  
  @chat_filter_invites.command(name="whitelist", brief="manage server", aliases=['wl', 'exempt'])
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_guild=True)
  async def chat_filter_invites_whitelist(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    if not mod: 
      return await ctx.send_error("Unable to find the **invites filter** automod rule. Please clear it and create a new one") 
    ...
    await ctx.send_success(f"{channel.mention} is now **exempted** from the invites filter")
  
  @chat_filter_invites.command(name="unwhitelist", brief="manage server", aliases=['uwl'])
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_guild=True)
  async def chat_filter_invites_unwhitelist(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    await ctx.send_success(f"{channel.mention} removed from the invites filter exempted channels")
  ...
  async def chat_filter_invites_whitelisted(self, ctx: PretendContext):
    ...
    await ctx.paginate(
      ...
      author={"name": self.bot.user.name, "icon_url": self.bot.user.display_avatar.url}
    ) 
  ...
  async def chat_filter_words_add(self, ctx: PretendContext, *, word: str): 
    ...
    check = await self.bot.db.fetchrow("SELECT rule_id FROM filter WHERE guild_id = $1 AND mode = $2", ctx.guild.id, "words")
    if not check: 
      trigger = AutoModTrigger(
        type=AutoModRuleTriggerType.keyword, 
        keyword_filter=["*" + word + "*"]
      ...
      mod = await ctx.guild.create_automod_rule(
        ...
        trigger=trigger, 
        enabled=True, 
        actions=[AutoModRuleAction(custom_message=f"Message blocked by {self.bot.user.name} for containing a word that cannot be used")],
        reason="Filter words rule created")   
      
      await self.bot.db.execute("INSERT INTO filter VALUES ($1,$2,$3)", ctx.guild.id, "words", mod.id)
      return await ctx.send_success(f"Created **word filter** with the word **{word}**") 
    else: 
      mod = await ctx.guild.fetch_automod_rule(check[0])
      if not mod: return await ctx.send_error("Unable to find the **words filter** automod rule. Please clear it and create a new one")
  ...
  async def chat_filter_words_remove(self, ctx: PretendContext, *, word: str): 
    ...
    if not mod: 
      return await ctx.send_error("Unable to find the **words filter** automod rule. Please clear it and create a new one")
    
    filters = mod.trigger.keyword_filter
    
    if not "*" + word + "*" in filters: 
      return await ctx.send_warning(f"The word **{word}** is not in the word filter list")
  ...
  async def chat_filter_words_clear(self, ctx: PretendContext): 
   ...
   await self.bot.db.execute("DELETE FROM filter WHERE guild_id = $1 AND mode = $2", ctx.guild.id, "words")
  ...
  async def chat_filter_words_list(self, ctx: PretendContext): 
   ...
   results = await self.bot.db.fetchrow("SELECT rule_id FROM filter WHERE guild_id = $1 AND mode = $2", ctx.guild.id, "words")
   ...
   await ctx.paginate(filters, title=f"Filtered words ({len(filters)})", author={"name": self.bot.user.name, "icon_url": self.bot.user.display_avatar.url})  

  @chat_filter_words.command(name="whitelist", brief="manage server", aliases=['wl', 'exempt'])
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_guild=True)
  async def chat_filter_words_whitelist(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    await mod.edit(
      exempt_channels=channels, 
      reason=f"Word filter rule edited by {ctx.author}"
    )
    await ctx.send_success(f"{channel.mention} is now **exempted** from the word filter")
  
  @chat_filter_words.command(name="unwhitelist", brief="manage server", aliases=['uwl'])
  @has_guild_permissions(manage_guild=True)
  @bot_has_guild_permissions(manage_guild=True)
  async def chat_filter_words_unwhitelist(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    await ctx.send_success(f"{channel.mention} removed from the word filter exempted channels")
  ...
  async def chat_filter_words_whitelisted(self, ctx: PretendContext): 
    ...
    await ctx.paginate([f"<#{c}>" for c in mod.exempt_channel_ids], "Words filter whitelisted", author={"name": self.bot.user.name, "icon_url": self.bot.user.display_avatar.url})   

...
class Events(Cog):
  ...
  async def test_message(self, ctx: PretendContext, channel: TextChannel) -> Message: 
    ...
    if not perms.send_messages or not perms.embed_links: 
      raise BadArgument(f"I do not have permissions to send the {table} message in {channel.mention}")
  ...
  async def on_guild_channel_delete(self, channel: abc.GuildChannel):
    if channel.type.name == "text":
      for q in ['welcome', 'boost', 'leave']:
        await self.bot.db.execute(f"DELETE FROM {q} WHERE channel_id = $1", channel.id)

  @group(
   invoke_without_command=True, 
   aliases=['greet', 'wlc', 'welc']
  )
  ...
  @query_limit("welcome")
  async def welcome_add(self, ctx: PretendContext, channel: TextChannel, *, code: str):
  ...
  async def welcome_remove(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    return await ctx.send_success(f"Deleted the welcome message from {channel.mention}")
  ...
  async def welcome_reset(self, ctx: PretendContext):
    ...
    if len(check) == 0:
      return await ctx.send_error("You have **no** welcome messages in this server")

...
from discord.ext.commands import Cog, hybrid_command, command, Author, cooldown, BucketType, CommandError, CommandOnCooldown 
...
class Economy(Cog): 
  ...
  @hybrid_command()
  @create_account()
  async def transfer(self, ctx: PretendContext, amount: CashAmount, *, member: EligibleEconomyMember):
   """
   Transfer cash to a member
   """
      
   embed = discord.Embed(color=self.color, description=f"{self.emoji} {ctx.author.mention} are you sure you want to transfer **{amount}** {self.cash} to {member.mention}")
  ...
  async def dice(self, ctx: PretendContext, amount: CashAmount): 
    ...
    async with self.locks[ctx.author.id]:
      ...
      if amount < 20:
       return await ctx.send_error(f"You cannot bet less than **20** {self.card}")
      ...
      if user_dice > bot_dice: 
        ...
        await self.bot.db.execute(
         ...
         """, 
         round(cash+amount, 2),
         int((datetime.datetime.now() + datetime.timedelta(seconds=10)).timestamp()),
         ctx.author.id
        )
  ...
  async def daily(self, ctx: PretendContext):
    ...
    async with self.locks[ctx.author.id]: 
      ...
      return await ctx.economy_send(f"You have claimed **{round(newcash, 2)}** {self.cash} {'+20%' if donor else ''}\nCome back **tomorrow** to claim again") 
  ...
  async def withdraw(self, ctx: PretendContext, amount: CardAmount):
    ...
    async with self.locks[ctx.author.id]:
      ...
      return await ctx.economy_send(f"Withdrawed **{self.humanize_number(amount)}** {self.card}")
  ...
  async def deposit(self, ctx: PretendContext, amount: CashAmount):
   ...
   async with self.locks[ctx.author.id]:
    ...
    return await ctx.economy_send(f"Deposited **{self.humanize_number(amount)}** {self.cash}")
  ...
  async def coinflip(self, ctx: PretendContext, amount: CashAmount, bet: str):
    ...
    async with self.locks[ctx.author.id]:   
      ...
      if amount < 20:
        return await ctx.send_error(f"You cannot bet less than **20** {self.cash}")
  ...
  async def work(self, ctx: PretendContext):
   ...
   await ctx.economy_send(f"You were working as **a {random.choice(self.jobs)}** and got **{received}** {self.cash}")
  
  @work.error 
  async def on_command_error(self, ctx: PretendContext, error: CommandError):
   if isinstance(error, CommandOnCooldown):
    return await ctx.economy_send(f"You have to wait **{humanize.precisedelta(datetime.timedelta(seconds=error.retry_after), format='%0.0f')}** to work again")
  ...
  async def leaderboard(self, ctx: PretendContext):
    ...
    to_show = [
     ...
     for check in sorted_results if self.bot.get_user(check['user_id'])
    ][:50]
  ...
  async def reset(self, ctx: PretendContext):
    ...
    if check := await self.bot.db.fetchrow("SELECT * FROM economy WHERE user_id = $1", ctx.author.id):

      async def yes_callback(interaction: discord.Interaction):
        await interaction.client.db.execute("DELETE FROM economy WHERE user_id = $1", interaction.user.id)
        embed = discord.Embed(
      ...
      async def no_callback(interaction: discord.Interaction):
        embed = discord.Embed(
          ...
          description=f"Cancelling action..."
        )
  ...
  async def balance(self, ctx: PretendContext, *, member: discord.Member=Author): 
    ...
    if not check: 
      return await ctx.send_error(f"Member doesn't have any **credits** {self.cash}") 
    
    daily = "Available"
    if check['daily']:
      if datetime.datetime.now().timestamp() < check['daily']:
        daily = self.bot.humanize_date(datetime.datetime.fromtimestamp(check['daily']))

...
from discord import Interaction, Embed, Member, User, AuditLogAction, Guild, TextChannel, Message, Role
...
class Antinuke(Cog):
    ...
    async def change_antinuke_owner(self, before: Guild, after: Guild):
      if before.owner_id != after.owner_id:
        if await self.bot.db.fetchrow("SELECT * FROM antinuke WHERE guild_id = $1 and owner_id = $2", before.id, before.owner_id):
      ...
      if self.bot.an.get_bot_perms(member.guild):
       if await self.bot.an.is_module("new accounts", member.guild):
         if not await self.joined_whitelist(member):
          res = await self.bot.db.fetchval("SELECT threshold FROM antinuke_modules WHERE guild_id = $1 AND module = $2", member.guild.id, "new accounts")
          if (datetime.datetime.now() - datetime.datetime.fromtimestamp(member.created_at.timestamp())).total_seconds() < res:
            tasks = [await self.bot.an.decide_punishment("new accounts", member, f"Account younger than {humanfriendly.format_timespan(res)}")] 
            action_time = datetime.datetime.now()
      ...
      if self.bot.an.get_bot_perms(member.guild):
       if member.public_flags.spammer:
        if await self.bot.an.is_module("spammer", member.guild):
          if not await self.joined_whitelist(member):
            tasks = [await self.bot.an.decide_punishment("spammer", member, f"Account flagged as spammer by discord")]
            action_time = datetime.datetime.now()
            check = await self.bot.db.fetchrow("SELECT owner_id, logs FROM antinuke WHERE guild_id = $1", member.guild.id)
            await self.bot.an.take_action(
          ...
          async for entry in channel.guild.audit_logs(limit=1, action=AuditLogAction.channel_create):
           if self.bot.an.check_hieracy(entry.user, channel.guild.me):
            if not await self.bot.an.is_whitelisted(entry.user):
              ...
              if await self.bot.an.check_threshold('channel create', entry.user):
        ...
        if await self.bot.an.is_module('channel delete', channel.guild):
          async for entry in channel.guild.audit_logs(limit=1, action=AuditLogAction.channel_delete):
            if self.bot.an.check_hieracy(entry.user, channel.guild.me):
              if not await self.bot.an.is_whitelisted(entry.user):
                await channel.clone()
                if await self.bot.an.check_threshold('channel delete', entry.user):
                  ...
                  if not cache: 
                    ...
                    tasks = [await self.bot.an.decide_punishment('channel delete', entry.user, "Deleting channels")]
                    action_time = datetime.datetime.now()
                    check = await self.bot.db.fetchrow("SELECT owner_id, logs FROM antinuke WHERE guild_id = $1", channel.guild.id)
                    await self.bot.an.take_action(
                      'Deleting channels',
      ...
      if self.bot.an.get_bot_perms(role.guild):
        if await self.bot.an.is_module('role delete', role.guild):
          async for entry in role.guild.audit_logs(limit=1, action=AuditLogAction.role_delete):
            if not await self.bot.an.is_whitelisted(entry.user) and self.bot.an.check_hieracy(entry.user, role.guild.me):
              ...
              if not cache: 
                ...
                tasks = [await self.bot.an.decide_punishment('role delete', entry.user, 'Deleting roles')]
                action_time = datetime.datetime.now()
    ...
    async def on_member_role_give(self, before: Member, after: Member):
      if len(before.roles) < len(after.roles):
        roles = [r for r in after.roles if not r in before.roles and r.is_assignable()]
        if len(roles) == 0:
          return
          ...
          if self.bot.an.get_bot_perms(before.guild):
            if await self.bot.an.is_module("role giving", before.guild):
              async for entry in after.guild.audit_logs(limit=1, action=AuditLogAction.member_role_update):
                if not await self.bot.an.is_whitelisted(entry.user): 
                 if self.bot.an.check_hieracy(entry.user, before.guild.me): 
                  if not self.bot.cache.get(f"role-give-{before.guild.id}"):
                    await self.bot.cache.set(f'role-give-{before.guild.id}', True, 5)
                    tasks = [
                      after.edit(roles=[r for r in before.roles if r.is_assignable() and r.is_bot_managed()], reason="Roles being reverted"),
                      await self.bot.an.decide_punishment("role giving", entry.user, "Giving roles with dangerous permissions")
                    ]                  
            ...
            if not await self.bot.an.is_whitelisted(entry.user):
              ...
              if await self.bot.an.check_threshold('ban', entry.user):
                ...
                if not cache: 
                  ...
                  tasks = [await self.bot.an.decide_punishment('ban', entry.user, 'Banning members')]
                  action_time = datetime.datetime.now()
                  check = await self.bot.db.fetchrow("SELECT owner_id, logs FROM antinuke WHERE guild_id = $1", guild.id)
                  await self.bot.an.take_action(
                   'Banning members',
    ...
    async def on_mass_mention(self, message: Message):
      if message.guild:
        if not message.is_system():
          if message.mention_everyone or any([len(r.members) > 10 for r in message.role_mentions]) or self.big_role_mention(message.role_mentions): 
           if await self.bot.an.is_module("mass mention", message.guild):
              ...
              if message.author.discriminator != "0000":
                ...
                tasks.append(await self.bot.an.decide_punishment("mass mention", message.author, "Mass mention"))
              
              if message.webhook_id: 
                webhook = [w for w in await message.channel.webhooks() if w.id == message.webhook_id]
              ...
              if not cache:
                ...
                action_time = datetime.datetime.now()
                check = await self.bot.db.fetchrow("SELECT owner_id, logs FROM antinuke WHERE guild_id = $1", message.guild.id)
                await self.bot.an.take_action(
                 "Mass mention",
                 message.author, 
                 tasks, 
    ...
    async def antinuke_reset(self, ctx: PretendContext):
        ...
        await ctx.confirmation_send("Are you sure you want to **disable** antinuke?", yes_func=yes_callback, no_func=no_callback)   
    
    @antinuke.command(
      name="status",
      aliases=['settings', 'config', 'stats']
    )
    @antinuke_configured()
    @admin_antinuke()
    async def an_status(self, ctx: PretendContext):
      """check what is enabled in your antinuke system"""
      results = await self.bot.db.fetch("SELECT module FROM antinuke_modules WHERE guild_id = $1", ctx.guild.id)
      if not results: 
        return await ctx.send_warning("There is **no** module enabled")
      
      embed = Embed(color=self.bot.color, description="\n".join(f"{result['module'].capitalize()} protection {self.bot.yes}" for result in results))
      embed.set_author(name=f"{ctx.guild.name}'s antinuke configuration", icon_url=ctx.guild.icon)
    ...
    async def antinuke_whitelisted(self, ctx: PretendContext):
       ...
       check = await self.bot.db.fetchrow("SELECT owner_id, whitelisted FROM antinuke WHERE guild_id = $1", ctx.guild.id)
       content = [f"<@!{check['owner_id']}> <a:crown:1021829752782323762>"]
       
       if check['whitelisted']:
        whitelisted = [w for w in json.loads(check['whitelisted']) if self.bot.get_user(w)]
        content.extend([f"<@!{wl}> {'<:ClydeBot:1036611645713158254>' if self.bot.get_user(wl).bot else ''}" for wl in whitelisted])
    ...
    async def antinuke_logs(self, ctx: PretendContext, *, channel: TextChannel=None):
      ...
      if not channel: 
        ...
        return await ctx.send_success(f"Set the antinuke logs channel to {channel.mention}")
    ...
    @an_channelremove.command(
      name="enable",
      brief="antinuke admin"
    )
    @antinuke_configured()
    @admin_antinuke()
    async def an_channelremove_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment):
    ...
    async def an_channelremove_disable(self, ctx: PretendContext):
      ...
      await self.bot.db.execute("DELETE FROM antinuke_modules WHERE guild_id = $1 AND module = $2", ctx.guild.id, "channel de;ete")
    ...
    @an_channelcreate.command(
      name="enable",
      brief="antinuke admin"
    )
    @antinuke_configured()
    @admin_antinuke()
    async def an_channelcreate_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment):
    ...
    @an_roledelete.command(
      name="enable",
      brief="antinuke admin"
    )
    @antinuke_configured()
    @admin_antinuke()
    async def an_roledelete_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment):
    ...
    @an_rolecreate.command(
      name="enable",
      brief="antinuke admin"
    )
    @antinuke_configured()
    @admin_antinuke()
    async def an_rolecreate_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment): 
    ...
    @antinuke_configured()
    @admin_antinuke()
    async def an_kick_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment):
    ...
    @antinuke_configured()
    @admin_antinuke()
    async def an_ban_enable(self, ctx: PretendContext, threshold: int, punishment: Punishment):
    ...
    async def an_massmention_enable(self, ctx: PretendContext, punishment: Punishment):
      ...
      check = await self.bot.db.fetchrow("SELECT * FROM antinuke_modules WHERE module = $1 AND guild_id = $2", "mass mention", ctx.guild.id)  
      if not check: 
       args = ["INSERT INTO antinuke_modules VALUES ($1,$2,$3,$4)", ctx.guild.id, "mass mention", punishment, None]
      else: 
       args = ["UPDATE antinuke_modules SET punishment = $1 WHERE guild_id = $2 AND module = $3", punishment, ctx.guild.id, "mass mention"]

      await self.bot.db.execute(*args)
      return await ctx.send_success(f"Enabled **anti mass mention** protection\nPunishment: **{punishment}**")  
    ...
    async def an_massmention_disable(self, ctx: PretendContext):
      ...
      await self.bot.db.execute("DELETE FROM antinuke_modules WHERE guild_id = $1 AND module = $2", ctx.guild.id, "mass mention")
    ...
    async def an_spammer_enable(self, ctx: PretendContext, punishment: Punishment):
      ...
      check = await self.bot.db.fetchrow("SELECT * FROM antinuke_modules WHERE module = $1 AND guild_id = $2", "spammer", ctx.guild.id)  
      if not check: 
       args = ["INSERT INTO antinuke_modules VALUES ($1,$2,$3,$4)", ctx.guild.id, "spammer", punishment, None]
      else: 
       args = ["UPDATE antinuke_modules SET punishment = $1 WHERE guild_id = $2 AND module = $3", punishment, ctx.guild.id, "spammer"]

      await self.bot.db.execute(*args)
      return await ctx.send_success(f"Enabled **anti spammer** protection\nPunishment: **{punishment}**")
    ...
    async def an_spammer_disable(self, ctx: PretendContext):
      ...
      check = await self.bot.db.fetchrow("SELECT * FROM antinuke_modules WHERE module = $1 AND guild_id = $2", "spammer", ctx.guild.id)    
      if not check: 
        return await ctx.send_warning("Spammer accounts protection is **not** enabled")
      
      await self.bot.db.execute("DELETE FROM antinuke_modules WHERE guild_id = $1 AND module = $2", ctx.guild.id, "spammer")
    ...
    @antinuke_configured()
    @admin_antinuke()
    async def an_newaccs_enable(self, ctx: PretendContext, time: ValidTime, punishment: Punishment):
      """enable the new account protection"""
      if punishment == "strip":
        return await ctx.send_error("**Strip** cannot be a punishment in this case")
      
      check = await self.bot.db.fetchrow("SELECT * FROM antinuke_modules WHERE module = $1 AND guild_id = $2", "new accounts", ctx.guild.id)  
      if not check: 
       args = ["INSERT INTO antinuke_modules VALUES ($1,$2,$3,$4)", ctx.guild.id, "new accounts", punishment, time]
      else: 
       args = ["UPDATE antinuke_modules SET punishment = $1, threshold = $4 WHERE guild_id = $2 AND module = $3", punishment, ctx.guild.id, "new accounts", time]
    ...
    async def an_newaccs_disable(self, ctx: PretendContext):
      ...
      await self.bot.db.execute("DELETE FROM antinuke_modules WHERE guild_id = $1 AND module = $2", ctx.guild.id, "new accounts")
    ...
    @antinuke_configured()
    @admin_antinuke()
    async def antinuke_whitelist(self, ctx: PretendContext, *, member: Union[User, Member]):
       ...
       return await ctx.send_success(f"Whitelisted {member.mention}. Now they can join the server")
    ...
    @antinuke_configured()
    @admin_antinuke()
    async def antinuke_wl_remove(self, ctx: PretendContext, *, member: Union[User, Member]):
      ...
      if whitelisted: 
        ...
        if not member.id in whitelisted: 
           return await ctx.send_warning("This member is **not** antinuke whitelisted")
        
        whitelisted.remove(member.id)
        await self.bot.db.execute("UPDATE antinuke SET whitelisted = $1 WHERE guild_id = $2", json.dumps(whitelisted), ctx.guild.id)
        return await ctx.send_success(f"Unwhitelisted {member.mention} from antinuke") 
    ...
    async def an_admin_add(self, ctx: PretendContext, *, member: Member): 
       ...
       if member.bot: 
         return await ctx.send("Why would a bot be an antinuke admin? They cannot manage the settings anyways -_-")

...
class Responders(Cog):
  ...
  @autoreact.command(name="add", brief="manage server", usage="example: ;autoreact add skull, 💀")  
  ...
  async def autoreact_add(self, ctx: PretendContext, *, content: str):
   ...
   unicode_regex = re.compile("[""\U0001F1E0-\U0001F1FF""\U0001F300-\U0001F5FF""\U0001F600-\U0001F64F""\U0001F680-\U0001F6FF""\U0001F700-\U0001F77F""\U0001F780-\U0001F7FF""\U0001F800-\U0001F8FF""\U0001F900-\U0001F9FF""\U0001FA00-\U0001FA6F""\U0001FA70-\U0001FAFF""\U00002702-\U000027B0""\U000024C2-\U0001F251""]+")
   reactions = [c.strip() for c in con[1].split(" ") if custom_regex.match(c) or unicode_regex.match(c)]
   ...
   if not check: 
    await self.bot.db.execute("INSERT INTO autoreact VALUES ($1,$2,$3)", ctx.guild.id, trigger, json.dumps(reactions))  
   else:
  ...
  async def autoreact_list(self, ctx: PretendContext): 
   ...
   if not check: 
    return await ctx.send_error("There are no autoreactions available in this server")
  ...
  async def ar_remove(self, ctx: PretendContext, *, trigger: str):
   ...
   if not check: 
    return await ctx.send_error("There is no autoresponder with the trigger you have provided")

...
class Boosterrole(Cog): 
  ...
  async def br_reset(self, ctx: PretendContext): 
    ...
    async def yes_callback(interaction: Interaction):
      ...
      return await interaction.response.edit_message(
        embed=Embed(color=self.bot.yes_color, description=f"{self.bot.yes} {ctx.author.mention}: Booster role module cleared"), 
        view=None
      )
  ...
  async def br_award_add(self, ctx: PretendContext, *, role: NewRoleConverter):
    ...
    if await self.bot.db.fetchrow("SELECT * FROM br_award WHERE guild_id = $1 AND role_id = $2", ctx.guild.id, role.id):
      return await ctx.send_warning("This role is **already** a booster role award")
    
    await self.bot.db.execute("INSERT INTO br_award VALUES ($1,$2)", ctx.guild.id, role.id)
  ...
  async def br_award_remove(self, ctx: PretendContext, *, role: NewRoleConverter):
    ...
    if not await self.bot.db.fetchrow("SELECT * FROM br_award WHERE guild_id = $1 AND role_id = $2", ctx.guild.id, role.id):
      return await ctx.send_warning("This role is **not** a booster role award")
    
    await self.bot.db.execute("DELETE FROM br_award WHERE guild_id = $1 AND role_id = $2", ctx.guild.id, role.id)
    return await ctx.send_success(f"Removed {role.mention} from the booster role awards")
  ...
  async def br_award_list(self, ctx: PretendContext):
    ...
    if results := await self.bot.db.fetch("SELECT role_id FROM br_award WHERE guild_id = $1", ctx.guild.id):
  ...
  async def br_create(self, ctx: PretendContext, *, name: str=None):
    ...
    if not ctx.author.premium_since: 
      return await ctx.send_warning("You have to **boost** this server to be able to use this command")
  ...
  async def br_name(self, ctx: PretendContext, *, name: str):
    ...
    if len(name) > 32: 
      return await ctx.send_warning("The booster role name cannot have more than **32** characters")
  ...
  async def br_icon(self, ctx: PretendContext, *, emoji: Union[PartialEmoji, str]): 
    ...
    return await ctx.send_success(f"Booster role icon succesfully changed to **{emoji.name if isinstance(emoji, PartialEmoji) else emoji}**")

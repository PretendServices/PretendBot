...
class BlackTea:
   ...
   async def lost_a_life(self, member: int, reason: str, channel: TextChannel): 
    ...
    if reason == "timeout": 
     await self.send_embed(channel, f"⏰ <@{member}> time is up! **{3-int(self.lifes[f'{channel.guild.id}'][f'{member}'])}** lifes left..")
    
    elif reason == "wrong":
     await self.send_embed(channel, f"💥 <@{member}> wrong answer! **{3-int(self.lifes[f'{channel.guild.id}'][f'{member}'])}** lifes left..")  
    ...
    if self.lifes[f'{channel.guild.id}'][f'{member}'] == 3: 
      ...
      if not check: 
        await self.bot.db.execute("INSERT INTO gamestats VALUES ($1,$2,$3,$4,$5)", member, 'blacktea', 0, 1, 1) 
      else: 
   ...
   async def add_loser(self, player: int): 
    check = await self.bot.db.fetchrow('SELECT * FROM gamestats WHERE user_id = $1 AND game = $2', player, 'blacktea')
    if not check: 
      return await self.bot.db.execute("INSERT INTO gamestats VALUES ($1,$2,$3,$4,$5)", player, 'blacktea', 0, 1, 1)
    else: 
   ...
   async def add_winner(self, player: int): 
    check = await self.bot.db.fetchrow("SELECT * FROM gamestats WHERE user_id = $1 AND game = $2", player, 'blacktea')
    if not check: 
      return await self.bot.db.execute("INSERT INTO gamestats VALUES ($1,$2,$3,$4,$5)", player, 'blacktea', 1, 0, 1)
    else: 
      return await self.bot.db.execute(
        """
        UPDATE gamestats 
...
class Fun(Cog): 
  ...
  async def stats_execute(self, ctx: PretendContext, member: User) -> Message:
    ...
    check = await self.bot.db.fetchrow("SELECT * FROM gamestats WHERE game = $1 AND user_id = $2", ctx.command.name, member.id)
  ...
  async def quran(self, ctx: PretendContext):
    ...
    result = await self.bot.session.get_json("https://api.alquran.cloud/v1/surah/40/en.sahih")
    ...
    embed = Embed(
      color = self.bot.color,
      description = f"**{number}:{numberInSurah}** {text}"
    )\
  ...
  async def bible(self, ctx: PretendContext):
    ...
    embed = Embed(color=self.bot.color, description=result['verse']['details']['text'])
  ...
  async def blacktea(self, ctx: PretendContext):
     ...
     for user in users: 
      ...
      try: 
        message = await self.bot.wait_for(
          'message', 
          check=lambda m: m.channel.id == ctx.channel.id and m.author.id == user, 
          timeout=10
        )
    ...
    if not check: 
      await self.bot.db.execute("INSERT INTO gamestats VALUES ($1,$2,$3,$4,$5)", member, 'blacktea', 1, 0, 1) 
    else: 
      await self.bot.db.execute(
        """
  ...
  @hybrid_group(invoke_without_command=True, description="check a member's stats for a certain game")
  ...
  async def pack(self, ctx: PretendContext, *, member: Member): 
   ...
   result = await self.bot.session.get_json("https://evilinsult.com/generate_insult.php?lang=en&type=json") 
   await ctx.send(f"{member.mention} {result['insult']}", allowed_mentions=AllowedMentions.none()) 
  ...
  async def eightball(self, ctx: PretendContext, *, question: str): 
   ...
   await ctx.reply(f"question: {question}{'?' if not question.endswith('?') else ''}\n{random.choice(['yes', 'no', 'never', 'most likely', 'absolutely', 'absolutely not', 'of course not'])}", allowed_mentions=discord.AllowedMentions.none())  
  ...
  async def bird(self, ctx: PretendContext): 
    ...
    await ctx.reply(file=File(fp=await self.bot.getbyte(data['file']), filename="bird.png"))
  ...
  async def dog(self, ctx: PretendContext):
    ...
    await ctx.reply(file=File(fp=await self.bot.getbyte(data['url']), filename=f"dog{data['url'][-4:]}"))
  ...
  async def cat(self, ctx: PretendContext):
    ...
    await ctx.reply(file=File(fp=await self.bot.getbyte(data['url']), filename="cat.png"))
  ...
  async def capybara(self, ctx: PretendContext):
    ...
    await ctx.reply(file=File(fp=await self.bot.getbyte(data['data']['url']), filename="cat.png"))
  ...
  async def rockpaperscisssors(self, ctx: PretendContext): 
    ...
    embed = Embed(color=self.bot.color, title="Rock Paper Scissors!", description="Click a button to play!")
  ...
  async def ship(self, ctx, member: Member):
    ...
    return await ctx.reply(f"**{ctx.author.name}** 💞 **{member.name}** = **{random.randrange(101)}%**")
  ...
  async def advice(self, ctx: PretendContext):
    ...
    data = orjson.loads(await self.bot.session.get_text("https://api.adviceslip.com/advice"))
  ...
  async def lick(self, ctx: PretendContext, *, member: Member=Author):
   ...
   try:
       lick = await asyncio.wait_for(self.bot.session.get_json("https://api.otakugifs.xyz/gif?reaction=lick&format=gif"), timeout=6)
   except asyncio.TimeoutError:
  ...
  async def kiss(self, ctx: PretendContext, *, member: Member):
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json("https://api.otakugifs.xyz/gif?reaction=kiss&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
  ...
  async def cuddle(self, ctx: PretendContext, *, member: Member):
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json("https://api.otakugifs.xyz/gif?reaction=cuddle&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
    embed = Embed(
      color=self.bot.color,
  ...
  async def hug(self, ctx: PretendContext, *, member: Member): 
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json(f"https://api.otakugifs.xyz/gif?reaction=hug&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
    embed = Embed(
      color=self.bot.color, 
  ...
  async def pat(self, ctx: PretendContext, *, member: Member):
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json(f"https://api.otakugifs.xyz/gif?reaction=pat&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
    embed = Embed(
      color=self.bot.color, 
  ...
  async def slap(self, ctx: PretendContext, *, member: Member): 
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json(f"https://api.otakugifs.xyz/gif?reaction=slap&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
    embed = Embed(
      color=self.bot.color, 
  ...
  async def laugh(self, ctx: PretendContext): 
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json(f"https://api.otakugifs.xyz/gif?reaction=laugh&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
    embed = Embed(
      color=self.bot.color, 
  ...
  async def cry(self, ctx: PretendContext):
    ...
    """
    try:
        lol = await asyncio.wait_for(self.bot.session.get_json(f"https://api.otakugifs.xyz/gif?reaction=cry&format=gif"), timeout=2)
    except asyncio.TimeoutError:
        return await ctx.send_error("There was an error with the API.")
  ...
  async def marriage(self, ctx: PretendContext, *, member: User=Author):
    ...
    check = await self.bot.db.fetchrow("SELECT * FROM marry WHERE $1 IN (author, soulmate)", member.id)
    if check is None:
      return await ctx.send_error(f"{'You are' if member == ctx.author else f'{member.mention} is'} not **married**")
  ...
  async def divorce(self, ctx: PretendContext):  
    ...
    async def button1_callback(interaction: Interaction) -> None:
      member = await self.bot.fetch_user(check['author'] if check['author'] != interaction.user.id else check['soulmate'])
      await interaction.client.db.execute("DELETE FROM marry WHERE $1 IN (author, soulmate)", interaction.user.id)                    
      embe = Embed(color=interaction.client.color, description=f"**{interaction.user.name}** divorced with their partner")
        
      try:
        await member.send(f"💔 It seems like your partner **{interaction.user}** decided to divorce :(. Your relationship with them lasted **{humanize.precisedelta(datetime.datetime.fromtimestamp(int(check['time'])), format=f'%0.0f')}**")
      except: 
        pass 
    ...
    async def button2_callback(interaction: Interaction) -> None:
      embe = Embed(color=interaction.client.color, description=f"**{interaction.user.name}** you changed your mind")
      await interaction.response.edit_message(content=None, embed=embe, view=None)

...
class Music(Cog): 
  ...
  async def music_send(self, ctx: PretendContext, message: str) -> Message: 
    ...
    embed = Embed(color=self.bot.color, description=f"🎵 {ctx.author.mention}: {message}")
  ...
  async def loop(self, ctx: PretendContext):
   ...
   if not player.loop: 
    player.loop = True 
    return await self.music_send(ctx, f"Looping [**{player.current_track.title}**]({player.current_track.uri})") 
   else: 
    player.loop = False 
    return await self.music_send(ctx, f"Removed the loop for [**{player.current_track.title}**]({player.current_track.uri})")
  ...
  async def play(self, ctx: PretendContext, *, query: str):
   ...
   if isinstance(results, pomice.Playlist):
    ...
    track = None
    await self.music_send(ctx, f"Added **{len(results.tracks)}** songs to the queue")
   else:
    track = results[0] 

...
class Config(Cog): 
  ...
  async def embed_json(self, member: Member, attachment: Attachment): 
   ...
   try:
    ...
    if message['data'].get('content'):
     content = EmbedBuilder().embed_replacement(member, message['data'].get("content"))
    else:
      content = None
    embeds = []
    for embed in message['data']['embeds']: 
     e = json.loads(EmbedBuilder().embed_replacement(member, str(embed).replace("\'", "\"")))
  ...
  async def usernames_add(self, ctx: PretendContext, *, channel: TextChannel):
    ...
    check = await self.bot.db.fetchrow("SELECT * FROM username_track WHERE guild_id = $1", ctx.guild.id)
    if check: 
    ...
    await self.bot.db.execute("INSERT INTO username_track VALUES ($1,$2)", ctx.guild.id, webhook.url)
  ...
  async def usernames_remove(self, ctx: PretendContext):
    ...
    await self.bot.db.execute("DELETE FROM username_track WHERE guild_id = $1", ctx.guild.id)
  ...
  async def disablecmd(self, ctx: PretendContext, *, command: ValidCommand): 
   ...
   check = await self.bot.db.fetchrow("SELECT * FROM disablecmd WHERE guild_id = $1 AND cmd = $2", ctx.guild.id, command)
   
   if check: 
    return await ctx.send_error("This command is **already** disabled") 
      
   await self.bot.db.execute("INSERT INTO disablecmd VALUES ($1,$2)", ctx.guild.id, command)
  ...
  async def enablecmd(self, ctx: PretendContext, *, command: ValidCommand): 
   ...
   await self.bot.db.execute("DELETE FROM disablecmd WHERE guild_id = $1 AND cmd = $2", ctx.guild.id, command)
  ...
  async def invoke_variables(self, ctx: PretendContext): 
    ...
    vars = '\n'.join([f'{m} - {Invoking(ctx).variables.get(m)}' for m in Invoking(ctx).variables])
    embed = Embed(color=self.bot.color, title="Invoke Variables", description=f">>> {vars}")
  ...
  async def autorole_add(self, ctx: PretendContext, *, role: NewRoleConverter):
    ...
    try: 
     await self.bot.db.execute("INSERT INTO autorole VALUES ($1,$2)", ctx.guild.id, role.id)    
  ...
  async def autorole_remove(self, ctx: PretendContext, *, role: Role): 
   ...
   if not await self.bot.db.fetchrow("SELECT * FROM autorole WHERE guild_id = $1 AND role_id = $2", ctx.guild.id, role.id):
     return await ctx.send_warning("This role is **not** configured as an autorole")
  ...
  @autorole.command(name="clear", description="clear the autorole list", brief="manage server")
  @has_guild_permissions(manage_guild=True)
  async def autorole_clear(self, ctx: PretendContext): 
   ...
   async def yes_callback(interaction: Interaction): 
    ...
    return await interaction.response.edit_message(embed=Embed(color=interaction.client.color, description="Cleared all the autoroles"), view=None)  
   
   async def no_callback(interaction: Interaction): 
    return await interaction.response.edit_message(embed=Embed(color=interaction.client.color, description="Aborting action...."), view=None) 
   
   await ctx.confirmation_send("Are you sure you want to **clear** the autoroles from this server?", yes_callback, no_callback)
  ...
  async def starboard_emoji(self, ctx: PretendContext, emoj: ValidEmoji):
   ...
   emoj = str(emoj)
   if not check: 
    await self.bot.db.execute("INSERT INTO starboard VALUES ($1,$2,$3,$4)", ctx.guild.id, None, emoj, 1)
   else: 
    await self.bot.db.execute("UPDATE starboard SET emoji = $1 WHERE guild_id = $2", emoj, ctx.guild.id)
  ...
  async def starboard_disable(self, ctx: PretendContext):
   ...
   async def yes_callback(interaction: Interaction) -> None: 
    ...
    return await interaction.response.edit_message(embed=Embed(color=interaction.client.yes_color, description=f"{interaction.client.yes} {interaction.user.mention}: Disabled starboard"), view=None)
   
   async def no_callback(interaction: Interaction) -> None: 
    return await interaction.response.edit_message(embed=Embed(color=interaction.client.yes_color, description=f"Aborting action.."), view=None)
   
   await ctx.confirmation_send("Are you sure you want to **disable** starboard", yes_callback, no_callback)

  @app_commands.command()
  async def confess(self, interaction: Interaction):
    """anonymously confess your thoughts"""
    re = await self.bot.db.fetchrow("SELECT * FROM confess_mute WHERE guild_id = $1 AND user_id = $2", interaction.guild.id, interaction.user.id)
    
    if re: 
      await interaction.warn("You are **muted** from sending confessions in this server", ephemeral=True)
    ...
    return await interaction.error("Confessions aren't enabled in this server", ephemeral=True)
  ...
  async def confessions_mute(self, ctx: PretendContext, *, confession: int): 
    ...
    re = await self.bot.db.fetchrow("SELECT * FROM confess_members WHERE guild_id = $1 AND confession = $2", ctx.guild.id, confession)
    ...
    if r: 
     return await ctx.send_warning("This **member** is **already** confession muted")
    
    await self.bot.db.execute("INSERT INTO confess_mute VALUES ($1,$2)", ctx.guild.id, member_id)
    return await ctx.send_success(f"The author of confession #{confession} is muted") 
  ...
  async def variables(self, ctx: PretendContext):
   ...
   embeds = [Embed(color=self.bot.color, title="Embed Variables", description=l) for l in [user_variables, guild_variables]]
  ...
  async def fp_perms(self, ctx: PretendContext):
    ...
    return await ctx.paginate(
      list(map(lambda p: p[0], ctx.author.guild_permissions)),
      "Valid permissions"
    )

  @fakepermissions.command(name="add", aliases=['append'], brief="administrator")
  @has_guild_permissions(administrator=True)
  async def fp_add(self, ctx: PretendContext, role: NewRoleConverter, permission: ValidPermission):
  ...
  @has_guild_permissions(administrator=True)
  async def fp_remove(self, ctx: PretendContext, role: NewRoleConverter, permission: ValidPermission):
  ...
  @has_guild_permissions(manage_guild=True) 
  @bump_enabled()
  async def bumpreminder_reminder(self, ctx: PretendContext, *, code: str="{embed}{color: #181a14}$v{description: 🕰️ Bump the server using `/bump`}$v{content: {user.mention}}"):  
  ...
  @reactionrole.command(name="clear", description="delete every reaction role in the server", brief="manage server")
  @has_guild_permissions(manage_guild=True)
  async def rr_clear(self, ctx: PretendContext):
   ...
   async def yes_callback(interaction: Interaction) -> None: 
    ...
    return await interaction.response.edit_message(embed=Embed(color=self.bot.yes_color, description=f"{self.bot.yes} {interaction.user.mention}: Cleared **all** reaction roles from the server"), view=None)
   
   async def no_callback(interaction: Interaction) -> None: 
    return await interaction.response.edit_message(embed=Embed(color=self.bot.yes_color, description=f"Aborting action.."), view=None)
   
   await ctx.confirmation_send("Are you sure you want to clear **all** reaction roles from the server? The reactions won't be removed", yes_callback, no_callback)
  ...
  async def rr_list(self, ctx: PretendContext): 
    ...
    return await ctx.paginate(
      [f"{result['emoji']} <@&{result['role_id']}> [**here**](https://discord.com/channels/{ctx.guild.id}/{result['channel_id']}/{result['message_id']})" for result in results],
      f"Reaction Roles ({len(results)})",
      {"name": ctx.guild.name, "icon_url": ctx.guild.icon}
    ) 

  @reactionrole.command(name="remove", brief="manage roles", usage="example: ;rr remove https://discord.com/channels/1134194462218788977/1134194463238000743/1136758099282231446 :skull:")
  @has_guild_permissions(manage_roles=True)
  async def rr_remove(self, ctx: PretendContext, message: ValidMessage, emoji: Union[Emoji, str]): 
   ...
   if not check: 
     return await ctx.send_error("No reaction role found for the message provided")
   ...
   return await ctx.send_success("Removed the reaction role from the message provided")  

  @reactionrole.command(name="add", brief="manage roles", usage="example: ;rr add https://discord.com/channels/1134194462218788977/1134194463238000743/1136758099282231446 :skull: dead chat")  
  @has_guild_permissions(manage_roles=True)
  async def rr_add(self, ctx: PretendContext, message: ValidMessage, emoji: Union[Emoji, str], *, role: NewRoleConverter):
    ...
    await self.bot.db.execute("INSERT INTO reactionrole VALUES ($1,$2,$3,$4,$5)", ctx.guild.id, message.channel.id, message.id, str(emoji), role.id)
    await message.add_reaction(emoji)
    return await ctx.send_success(f"Added reaction role [**here**]({message.jump_url})")
  ...
  async def editrole_name(self, ctx: PretendContext, role: NewRoleConverter, *, name: str): 
   """edit a role's name"""
  ...
  @bot_has_guild_permissions(manage_roles=True)
  @boosted_to(2)
  async def editrole_icon(self, ctx: PretendContext, role: NewRoleConverter, *, emoji: Union[PartialEmoji, str]):
   """edit a role's icon"""
   await role.edit(display_icon=await emoji.read() if isinstance(emoji, PartialEmoji) else emoji, reason=f"Role icon edited by {ctx.author}")
   return await ctx.send_success(f"Role icon succesfully changed to **{emoji.name if isinstance(emoji, PartialEmoji) else emoji}**")
  ...
  @has_guild_permissions(manage_roles=True)
  @bot_has_guild_permissions(manage_roles=True)
  async def editrole_color(self, ctx: PretendContext, role: NewRoleConverter, color: HexColor):
   """edit a role's color"""

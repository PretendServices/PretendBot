...
class Moderation(Cog): 
     ...
     if not await self.bot.an.is_whitelisted(member):
      ...
      if self.bot.an.check_hieracy(member, member.guild.me):
        ...
        if not cache:
          ...
          tasks = [await self.bot.an.decide_punishment(module, member, reason)]
  ...
  async def setjail(self, ctx: PretendContext):
   ...
   async with self.locks[ctx.guild.id]: 
    ...
    async with ctx.typing():
     ...
     await asyncio.gather(*[channel.set_permissions(role, view_channel=False) for channel in ctx.guild.channels])
  ...
  @bot_has_guild_permissions(manage_roles=True)
  @is_jail()
  async def jail(self, ctx: PretendContext, member: NoStaff, *, reason: str="No reason provided"): 
   """
   ...
   if not role: 
    return await ctx.send_error("Jail role **not found**. Please unset jail and set it back") 
   ...
   try: 
    await member.send(f"{member.mention}, you have been jailed in **{ctx.guild.name}** (`{ctx.guild.id}`) - {reason}! Wait for a staff member to unjail you")
   except: 
    pass
  ...
  @bot_has_guild_permissions(manage_roles=True)
  @is_jail()
  async def unjail(self, ctx: PretendContext, member: Member, *, reason: str="No reason provided"): 
   """
   ...
   roles = [ctx.guild.get_role(r) for r in json.loads(re[0]) if ctx.guild.get_role(r)]
  ...
  async def voicemute(self, ctx: PretendContext, *, member: NoStaff):
    ...
    await member.edit(
      mute=True, 
      reason=f"Member voice muted by {ctx.author}"
    )
  ...
  async def voiceunmute(self, ctx: PretendContext, *, member: NoStaff):
    ...
    await member.edit(
      mute=False,
      reason=f"Member voice unmuted by {ctx.author}"
    )
  ...
  async def voicedeafen(self, ctx: PretendContext, *, member: NoStaff):
    ...
    await member.edit(
      deafen=True, 
      reason=f"Member voice deafened by {ctx.author}"
    )
  ...
  async def voiceundeafen(self, ctx: PretendContext, *, member: NoStaff):
    ...
    await member.edit(
      deafen=False,
      reason=f"Voice undeafened by {ctx.author}"
    )
  ...
  async def purge(self, ctx: PretendContext, number: int, *, member: Member=None): 
   ...
   async with self.locks[ctx.channel.id]:
    ...
    await ctx.channel.purge(
      limit=number, 
      check=check,
      reason=f"Chat purged by {ctx.author}"
    )  
  ...
  async def unlock(self, ctx: PretendContext, *, channel: TextChannel=CurrentChannel):
   ...
   if channel.overwrites_for(ctx.guild.default_role).send_messages is True or channel.overwrites_for(ctx.guild.default_role).send_messages is None: 
  ...
  @has_guild_permissions(manage_channels=True)
  @bot_has_guild_permissions(manage_channels=True)
  async def slowmode(self, ctx: PretendContext, time: ValidTime, *, channel: TextChannel=CurrentChannel):
    """
    ...
    await channel.edit(
      slowmode_delay=time,
      reason=f'Slowmode invoked by {ctx.author}'
    ) 
    await ctx.send_success(f"Slowmode for {channel.mention} set to **{format_timespan(time)}**")  
  ...
  async def mute(self, ctx: PretendContext, member: NoStaff, time: ValidTime=3600, *, reason: str="No reason provided"): 
    """
    timeout a member
    """
    ...
    await member.timeout(utils.utcnow() + datetime.timedelta(seconds=time), reason=reason)
   
    if not await Invoking(ctx).send(member, reason):
      await ctx.send_success(f"Muted {member.mention} for {format_timespan(time)} - **{reason}**")
  
  @hybrid_command(brief="moderate members", aliases=['untimeout'])
  @has_guild_permissions(moderate_members=True)
  @bot_has_guild_permissions(moderate_members=True)
  async def unmute(self, ctx: PretendContext, member: NoStaff, *, reason: str="No reason provided"): 
    """
  ...
  async def ban(self, ctx: PretendContext, member: Union[Member, User], *, reason: str="No reason provided"):
   """
  ...
  async def kick(self, ctx: PretendContext, member: NoStaff, *, reason: str="No reason provided"):
   """
  ...
  @has_guild_permissions(ban_members=True)
  @bot_has_guild_permissions(ban_members=True)
  async def unban(self, ctx: PretendContext, member: User, *, reason: str="No reason provided"):
    """
  ...
  async def nickname(self, ctx: PretendContext, member: NoStaff, *, nick: ValidNickname): 
   """
  ...
  @has_guild_permissions(manage_messages=True)
  async def warn(self, ctx: PretendContext, member: NoStaff=None, *, reason: str="No reason provided"):
    if member is None: 
      return await ctx.create_pages() 
  ...
  async def warn_list(self, ctx: PretendContext, *, member: Member): 
    ...
    return await ctx.paginate(
     [f"{result['time']} by <@!{result['author_id']}> - {result['reason']}" for result in check],
  ...
  async def roleall(self, ctx: PretendContext, *, role: NewRoleConverter):
   ...
   async with self.role_lock[ctx.guild.id]:
    tasks = [m.add_roles(role, reason=f"Role all invoked by {ctx.author}") for m in ctx.guild.members if not role in m.roles]

...
class Donor(Cog): 
   ...
   if before.discriminator == "0": 
    if before.name != after.name:
      if not self.bot.cache.get("pomelo"): 
       await self.bot.cache.set("pomelo", [{"username": before.name, "time": utils.format_dt(datetime.datetime.now(), style="R")}])
      else: 
       lol = self.bot.cache.get("pomelo")
       lol.append({"username": before.name, "time": utils.format_dt(datetime.datetime.now(), style="R")})
   ...
   if str(before.nick) != str(after.nick):
    if nickname := await self.bot.db.fetchval("SELECT nickname FROM force_nick WHERE guild_id = $1 AND user_id = $2", before.guild.id, before.id):
     if after.nick != nickname:
      await after.edit(
        nick=nickname,
        reason="Force nickname applied to this member"
      )
    ...
    *, nickname: str
    ...
    if nickname.lower() == "none":
     if await self.bot.db.fetchrow("SELECT * FROM force_nick WHERE guild_id = $1 AND user_id = $2", ctx.guild.id, member.id):
      ...
      await member.edit(
       nick=None, 
       reason="Removed the force nickname from this member"
      )
  ...
  async def reskin_enable(self, ctx: PretendContext):
   ...
   if await self.bot.db.fetchrow("SELECT * FROM reskin_enabled WHERE guild_id = $1", ctx.guild.id):
    return await ctx.send_warning("Reskin is **already** enabled")
   
   await self.bot.db.execute("INSERT INTO reskin_enabled VALUES ($1)", ctx.guild.id)
  ...
  async def reskin_disable(self, ctx: PretendContext):
   ...
   if not await self.bot.db.fetchrow("SELECT * FROM reskin_enabled WHERE guild_id = $1", ctx.guild.id):
    return await ctx.send_warning("Reskin is **not** enabled")
   
   await self.bot.db.execute("DELETE FROM reskin_enabled WHERE guild_id = $1", ctx.guild.id)
  ...
  async def reskin_avatar(self, ctx: PretendContext, url: str=None):
   ...
   await self.bot.db.execute("UPDATE reskin SET avatar_url = $1 WHERE user_id = $2", url, ctx.author.id)

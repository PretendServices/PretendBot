...
from discord.ext.commands import Cog, hybrid_group, hybrid_command, CooldownMapping, BucketType, Author, has_guild_permissions 
...
class Leveling(Cog):
    ...
    async def give_rewards(self, member: Member, level: int):
      ...
      results = await self.bot.db.fetch("SELECT role_id FROM level_rewards WHERE guild_id = $1 AND level < $2", member.guild.id, level+1)
      if results: 
        tasks = [
    ...
    async def on_message(self, message: Message):
      if not message.author.bot: 
        if res := await self.bot.db.fetchrow("SELECT * FROM leveling WHERE guild_id = $1", message.guild.id): 
    ...
    async def on_guild_role_delete(self, role: Role):
      if await self.bot.db.fetchrow("SELECT * FROM level_rewards WHERE guild_id = $1 AND role_id = $2", role.guild.id, role.id):
        await self.bot.db.execute('DELETE FROM level_rewards WHERE role_id = $1 AND guild_id = $2', role.id, role.guild.id)
    ...
    async def level_disable(self, ctx: PretendContext):
      ...
      async def no_callback(interaction: Interaction):
        embed = Embed(
          ...
          description=f"Aborted the action..."
        )
        await interaction.response.edit_message(embed=embed, view=None) 
    ...
    @has_guild_permissions(manage_guild=True)
    @leveling_enabled()
    async def level_channel(self, ctx: PretendContext, *, channel: Optional[TextChannel]=None):
      """
    ...
    @has_guild_permissions(manage_guild=True) 
    @leveling_enabled()
    async def level_message(self, ctx: PretendContext, *, message: str="Good job, {user}! You leveled up to **Level {level}**"): 
      """
    ...
    @level_cmd.command(
      name="config",
      aliases=['settings', 'stats', 'statistics', 'status']
    )
    @leveling_enabled()
    async def level_config(self, ctx: PretendContext):
      ...
      embed = Embed(color=self.bot.color)
      embed.set_author(
        name=f"Level settings for {ctx.guild.name}",
        icon_url=ctx.guild.icon
      )

      embed.add_field(
        name="Level Channel",
        value=ctx.guild.get_channel(check['channel_id'])
      )
    ...
    async def level_leaderboard(self, ctx: PretendContext):
      ...
      await ctx.paginate(
        [f"**{ctx.guild.get_member(m['user_id']) or m['user_id']}** has level **{m['level']}** ({m['xp']:,} xp)" for m in members],
        "Level leaderboard",
    ...
    @level_rewards.command(
      name="add",
      brief="manage server"
    )
    @has_guild_permissions(manage_guild=True)
    async def level_rewards_add(self, ctx: PretendContext, level: int, *, role: NewRoleConverter):
    ...
    @has_guild_permissions(manage_guild=True)
    async def level_rewards_remove(self, ctx: PretendContext, *, role: NewRoleConverter):
    ...
    async def level_rewards_reset(self, ctx: PretendContext):
      ...
      await ctx.confirmation_send("Are you sure that you want to **remove** every reward saved in this server?", yes_callback, no_callback)
    ...
    async def level_rewards_list(self, ctx: PretendContext):
      ...
      await ctx.paginate(
        [f"{ctx.guild.get_role(r['role_id']).mention} for **Level {r['level']}**" for r in roles],

...
class Autopfp(commands.Cog): 
    ...
    async def autopfp_add(
        ...
        category: Literal['random', 'roadmen', 'girl', 'egirl', 'anime', 'ceinory'] = "random"
    ):
        """
        ...
        if not self.bot.autopfp_send: 
            ...
            asyncio.ensure_future(self.bot.autopfp())
    ...
    async def report(
        ...
        category: Literal['cute', 'mix', 'anime', 'girl', 'egirl', 'roadmen', 'ceinory'],
        image_id: str
    ):
        """
        ...
        file_path = os.path.join(directory, next(e for e in os.listdir(directory) if image_id == e[:-4]))
